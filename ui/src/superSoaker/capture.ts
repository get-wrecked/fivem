...
class SoakerUI {
  ...
  initialize() {
    ...
    try {
      ...
      const three = req && req('@citizenfx/three');
    ...
    window.addEventListener('message', (event) => {
      const req: SoakerRequest | undefined = (event.data && event.data.request) || undefined;
      if (!req) return;
      this.pending = req;
    });
  }
  ...
  private resize(three: any) {
    ...
    const { OrthographicCamera, Scene, PlaneBufferGeometry, Mesh, WebGLRenderTarget, LinearFilter, NearestFilter, RGBAFormat, UnsignedByteType } = three;
   ...
   */
  private async handleRequest(three: any, request: SoakerRequest) {
    let imageURL = '';
    ...
    if (this.available && this.renderer && this.rtTexture) {
      ...
      imageURL = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8Xw8AAgMBgYt3Wq8AAAAASUVORK5CYII=';
    }
    ...
    try {
      if (request.targetURL) {
        ...
        const getForm = () => {
          ...
          fd.append(request.targetField || 'file', dataURItoBlob(imageURL), `soaker.${request.encoding}`);
          return fd;
        };
        ...
        const resp = await fetch(request.targetURL, {
          ...
          mode: 'cors',
          headers,
          body: request.targetField ? getForm() : JSON.stringify({ data: imageURL, id: request.correlation }),
        });
